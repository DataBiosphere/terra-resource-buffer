...
public class BackwardsCompatibilityTest extends BaseUnitTest {
   ...
   * bio.terra.buffer.generated.model.PoolStatus}. We make API and internal use status(also DB) keep
   * in sync
   */
  @Test
  public void poolStatusDbWithApi() {
    // Make sure we won't forget to modify this test when we add/remove enums.
    assertEquals(2, PoolStatus.values().length);
    assertEquals(2, bio.terra.buffer.generated.model.PoolStatus.values().length);

    assertEquals(
        PoolStatus.ACTIVE.toString(),
        bio.terra.buffer.generated.model.PoolStatus.ACTIVE.toString());
    assertEquals(
        PoolStatus.DEACTIVATED.toString(),
        bio.terra.buffer.generated.model.PoolStatus.DEACTIVATED.toString());
  }

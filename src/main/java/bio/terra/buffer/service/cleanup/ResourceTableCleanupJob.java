...
public class ResourceTableCleanupJob {
  ...
  private final Logger logger = Logger.getLogger(ResourceTableCleanupJob.class.getName());
  ...
  public void cleanupResourceTable() {
    ...
    logger.info("Removing dead resource records older than " + config.getRetentionDays() + " days");
    var deletedResourceRecords =
        bufferDao.removeDeadResourceRecords(config.getRetentionDays(), config.getBatchSize());
    logger.info(
        "Resource table cleanup job removed = "
            + deletedResourceRecords
            + " dead resource records");

    // cleanup the cleanup_record table
    logger.info("Removing dead cleanup records older than " + config.getRetentionDays() + " days");
    var deletedCleanupRecords =
        bufferDao.removeDeadCleanupRecords(config.getRetentionDays(), config.getBatchSize());
    logger.info(
        "Resource table cleanup job removed = " + deletedCleanupRecords + " dead cleanup records");
